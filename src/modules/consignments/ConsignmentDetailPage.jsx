...
export default function ConsignmentDetailPage() {
  ...
  const shouldScrollToTracking =
    location.pathname.endsWith('/tracking') || searchParams.get('tab') === 'tracking'
  ...
  async function loadDetails() {
    setLoading(true)
    try {
      const [cnRes, trackRes, opsTrackRes, pickupRes] = await Promise.all([
  ...
  useEffect(() => {
    if (loading || !shouldScrollToTracking) return undefined
    const t = window.setTimeout(() => {
      document.getElementById('cn-tracking')?.scrollIntoView({ behavior: 'smooth', block: 'start' })
    }, 120)
    return () => window.clearTimeout(t)
  }, [loading, shouldScrollToTracking, cn])
  ...
  const actionMenuItems = {
    items: [
      ...
      },
      {
        ...
        onClick: () =>
          navigate(`/ops/reports/cn?id=${encodeURIComponent(activeCn)}&format=a5`),
      },
      {
        key: 'ticket',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        >
          ...
          <Button
            ...
            onClick={() => document.getElementById('cn-tracking')?.scrollIntoView({ behavior: 'smooth', block: 'start' })}
          >
            Jump to tracking ↓
          </Button>
        </div>

        <div id="cn-tracking" style={{ scrollMarginTop: 16 }}>
